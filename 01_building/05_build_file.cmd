...
TYPE "%building_blocks%\default\_version.txt" | FINDSTR /v # >"%output_dir%\default\htaccess_parameters_clean.txt"
:: Check parameter file for unwanted characters
FINDSTR /R "( ) & ' ` \"" "%output_dir%\default\htaccess_parameters_clean.txt" > NUL
IF NOT ERRORLEVEL 1 (
...
FOR /F "tokens=1,2 delims==" %%A IN ('FINDSTR /R /X /C:"[^=][^=]*=.*" "%output_dir%\default\htaccess_parameters_clean.txt" ') DO (
...
::
:: Remove comment lines
TYPE "%building_blocks%\%baseline%\_version.txt" | FINDSTR /v # >"%output_dir%\%baseline%\htaccess_parameters_clean.txt"
:: Check parameter file for unwanted characters
FINDSTR /R "( ) & ' ` \"" "%output_dir%\%baseline%\htaccess_parameters_clean.txt" > NUL
IF NOT ERRORLEVEL 1 (
	SET ERROR_MESSAGE=[ERROR] The parameter file contains unwanted characters, and cannot be parsed.
	GOTO ERROR_EXIT
)
:: Only parse the file if no unwanted characters were found
FOR /F "tokens=1,2 delims==" %%A IN ('FINDSTR /R /X /C:"[^=][^=]*=.*" "%output_dir%\%baseline%\htaccess_parameters_clean.txt" ') DO (
	SET buildparameter.%%A=%%B
)
...
FOR /f %%G in ('dir /b /A:-D "%building_blocks%\default\*.htaccess.txt"') DO (
...
::
COPY /b %input_filenames% %output_dir%\%baseline%\htaccess_constructed.txt																	

::
:: Inspiration: https://stackoverflow.com/questions/418916/delete-certain-lines-in-a-txt-file-via-a-batch-file
:: 
:: Remove the comment lines
TYPE "%output_dir%\%baseline%\htaccess_constructed.txt" | FINDSTR /v # >"%output_dir%\%baseline%\htaccess_clean.txt"

:: Create new header
ECHO #======================================================= >"%output_dir%\tmp_header.txt"
ECHO # .htaccess file created for: %baseline% >>"%output_dir%\tmp_header.txt"
ECHO # build version: %buildversion% >>"%output_dir%\tmp_header.txt"
ECHO # by script on : %dtStamp% >>"%output_dir%\tmp_header.txt"
ECHO # More information is available in ...\%baseline%\htaccess_%buildversion%_comments.txt >>"%output_dir%\tmp_header.txt"
ECHO #======================================================= >>"%output_dir%\tmp_header.txt"
ECHO # >>"%output_dir%\tmp_header.txt"

:: Create the htaccess.txt file
COPY /b %output_dir%\tmp_header.txt+%output_dir%\%baseline%\htaccess_clean.txt %output_dir%\%baseline%\htaccess_%buildversion%.txt

:: Add header to htaccess_constructed.txt file with comments
COPY /b %output_dir%\tmp_header.txt+%output_dir%\%baseline%\htaccess_constructed.txt %output_dir%\%baseline%\htaccess_%buildversion%_comments.txt
...
IF EXIST "%output_dir%\default\htaccess_parameters_clean.txt" ( DEL %output_dir%\default\htaccess_parameters_clean.txt )
